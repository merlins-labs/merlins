...
ifeq (cleveldb,$(findstring cleveldb,$(MERLINS_BUILD_OPTIONS)))
  build_tags += gcc
else ifeq (rocksdb,$(findstring rocksdb,$(MERLINS_BUILD_OPTIONS)))
  build_tags += gcc
endif
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=merlins \
...
ifeq (,$(findstring nostrip,$(MERLINS_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
ifneq ($(GO_MINOR_VERSION),20)
	@echo "ERROR: Go version 1.20 is required for this version of Merlins."
	exit 1
endif
...
build: check_version go.sum
	mkdir -p $(BUILDDIR)/
	GOWORK=off go build -mod=readonly  $(BUILD_FLAGS) -o $(BUILDDIR)/ $(GO_MODULE)/cmd/merlins
...
install: check_version go.sum
	GOWORK=off go install -mod=readonly $(BUILD_FLAGS) $(GO_MODULE)/cmd/merlins
...
install-with-autocomplete: check_version go.sum
	...
	if echo "$$PARENT_SHELL" | grep -q "zsh"; then \
		if ! grep -q ". <(merlins enable-cli-autocomplete zsh)" ~/.zshrc; then \
			echo ". <(merlins enable-cli-autocomplete zsh)" >> ~/.zshrc; \
			echo; \
		...
		if ! grep -q -e "\. <(merlins enable-cli-autocomplete bash)" -e '\[\[ -r "/opt/homebrew/etc/profile.d/bash_completion.sh" \]\] && \. "/opt/homebrew/etc/profile.d/bash_completion.sh"' ~/.bash_profile; then \
			brew install bash-completion; \
			echo '[ -r "/opt/homebrew/etc/profile.d/bash_completion.sh" ] && . "/opt/homebrew/etc/profile.d/bash_completion.sh"' >> ~/.bash_profile; \
			echo ". <(merlins enable-cli-autocomplete bash)" >> ~/.bash_profile; \
			echo; \
			echo; \
		...
		if ! grep -q ". <(merlins enable-cli-autocomplete bash)" ~/.bash_profile; then \
...
build-reproducible-amd64: go.sum
	mkdir -p $(BUILDDIR)
	$(DOCKER) buildx create --name furybuilder || true
	$(DOCKER) buildx use furybuilder
	$(DOCKER) buildx build \
		...
		-t merlins:local-amd64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f furybinary || true
	$(DOCKER) create -ti --name furybinary merlins:local-amd64
	$(DOCKER) cp furybinary:/bin/merlins $(BUILDDIR)/merlins-linux-amd64
	$(DOCKER) rm -f furybinary
...
build-reproducible-arm64: go.sum
	...
	$(DOCKER) buildx build \
		...
		-t merlins:local-arm64 \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f furybinary || true
	$(DOCKER) create -ti --name furybinary merlins:local-arm64
	$(DOCKER) cp furybinary:/bin/merlins $(BUILDDIR)/merlins-linux-arm64
	$(DOCKER) rm -f furybinary
...
draw-deps:
	...
	@goviz -i ./cmd/merlins -d 2 | dot -Tpng -o dependency-graph.png
...
# deletes any pre-existing Merlins containers before running.
#
...
test-e2e-ci:
	@VERSION=$(VERSION) MERLINS_E2E=True MERLINS_E2E_DEBUG_LOG=False MERLINS_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -p 4
...
test-e2e-debug: e2e-setup
	@VERSION=$(VERSION) MERLINS_E2E=True MERLINS_E2E_DEBUG_LOG=True MERLINS_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION) MERLINS_E2E_SKIP_CLEANUP=True go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -count=1
...
test-e2e-short: e2e-setup
	@VERSION=$(VERSION) MERLINS_E2E=True MERLINS_E2E_DEBUG_LOG=True MERLINS_E2E_SKIP_UPGRADE=True MERLINS_E2E_SKIP_IBC=True MERLINS_E2E_SKIP_STATE_SYNC=True MERLINS_E2E_SKIP_CLEANUP=True go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -count=1
...
docker-build-debug:
	@DOCKER_BUILDKIT=1 docker build -t merlins:${COMMIT} --build-arg BASE_IMG_TAG=debug --build-arg RUNNER_IMAGE=$(RUNNER_BASE_IMAGE_ALPINE) -f Dockerfile .
	@DOCKER_BUILDKIT=1 docker tag merlins:${COMMIT} merlins:debug

docker-build-e2e-init-chain:
	@DOCKER_BUILDKIT=1 docker build -t fanfury/merlins-e2e-init-chain:debug --build-arg E2E_SCRIPT_NAME=chain --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .

docker-build-e2e-init-node:
	@DOCKER_BUILDKIT=1 docker build -t merlins-e2e-init-node:debug --build-arg E2E_SCRIPT_NAME=node --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .
...
localnet-keys:
	. tests/localmerlins/scripts/add_keys.sh

localnet-init: localnet-clean localnet-build

localnet-build:
	@DOCKER_BUILDKIT=1 COMPOSE_DOCKER_CLI_BUILD=1 docker-compose -f tests/localmerlins/docker-compose.yml build

localnet-start:
	@STATE="" docker-compose -f tests/localmerlins/docker-compose.yml up

localnet-start-with-state:
	@STATE=-s docker-compose -f tests/localmerlins/docker-compose.yml up

localnet-startd:
	@STATE="" docker-compose -f tests/localmerlins/docker-compose.yml up -d

localnet-startd-with-state:
	@STATE=-s docker-compose -f tests/localmerlins/docker-compose.yml up -d

localnet-stop:
	@STATE="" docker-compose -f tests/localmerlins/docker-compose.yml down

localnet-clean:
	@rm -rfI $(HOME)/.merlins-local/

localnet-state-export-init: localnet-state-export-clean localnet-state-export-build 

localnet-state-export-build:
	@DOCKER_BUILDKIT=1 COMPOSE_DOCKER_CLI_BUILD=1 docker-compose -f tests/localmerlins/state_export/docker-compose.yml build

localnet-state-export-start:
	@docker-compose -f tests/localmerlins/state_export/docker-compose.yml up

localnet-state-export-startd:
	@docker-compose -f tests/localmerlins/state_export/docker-compose.yml up -d

localnet-state-export-stop:
	@docker-compose -f tests/localmerlins/docker-compose.yml down

localnet-state-export-clean: localnet-clean

# create 100 concentrated-liquidity positions in localmerlins at pool id 1
localnet-cl-create-positions:
	go run tests/cl-go-client/main.go --operation 0

# does 100 small randomized swaps in localmerlins at pool id 1
...
# next swap. localmerlins at pool id 1
