...
func (suite *StargateTestSuite) SetupTest() {
	...
	suite.ctx = suite.app.BaseApp.NewContext(false, tmproto.Header{Height: 1, ChainID: "merlins-1", Time: time.Now().UTC()})
}
...
func (suite *StargateTestSuite) TestStargateQuerier() {
	testCases := []struct {
		...
		{
			name: "happy path",
			path: "/merlins.epochs.v1beta1.Query/EpochInfos",
			requestData: func() []byte {
		...
		},
		{
			...
			path: "/merlins.gamm.v2.Query/SpotPrice",
			testSetup: func() {
		...
		},
		{
			...
			path: "/merlins.poolmanager.v1beta1.Query/SpotPrice",
			testSetup: func() {
				pk := ed25519.GenPrivKey().PubKey()
				sender := sdk.AccAddress(pk.Address())
				err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, sender, apptesting.DefaultAcctFunds)
		...
		},
		{
			...
			path: "/merlins.lockup.Query/AccountLockedLongerDuration",
			requestData: func() []byte {
		...
		},
		{
			...
			testSetup: func() {
				accAddr, err := sdk.AccAddressFromBech32("fury1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44")
				suite.Require().NoError(err)
			...
			requestData: func() []byte {
				bankrequest := banktypes.QueryAllBalancesRequest{
					Address: "fury1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44",
				}
		...
		},
		{
			...
			testSetup: func() {
				wasmbinding.SetWhitelistedQuery("/merlins.epochs.v1beta1.Query/EpochInfos",
