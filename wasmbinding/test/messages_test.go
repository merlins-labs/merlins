...
func TestCreateDenom(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformCreateDenom(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			merlins, ctx := SetupCustomApp(t, tokenCreator)

			err := wasmbinding.PerformCreateDenom(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasmbinding.ChangeAdmin(merlins.TokenFactoryKeeper, ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	merlins, ctx := SetupCustomApp(t, creator)

	// Create denoms for valid mint tests
	validDenom := bindings.CreateDenom{
		Subdenom: "MOON",
	}
	err := wasmbinding.PerformCreateDenom(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	err = wasmbinding.PerformCreateDenom(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	// lucky was broke
	balances := merlins.BankKeeper.GetAllBalances(ctx, lucky)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformMint(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasmbinding.PerformMint(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasmbinding.PerformMint(merlins.TokenFactoryKeeper, merlins.BankKeeper, ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasmbinding.PerformBurn(merlins.TokenFactoryKeeper, ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
