...
var (
	//go:embed "merlins-1-assetlist.json" "fury-test-5-assetlist.json"
	assetFS   embed.FS
	mainnetId = "merlins-1"
...
func loadAssetList(initClientCtx client.Context, cmd *cobra.Command, basedenomToIBC, IBCtoBasedenom bool) (map[string]DenomUnitMap, map[string]string) {
	...
	if chainId == mainnetId || chainId == "" {
		fileName = "cmd/merlins/cmd/merlins-1-assetlist-manual.json"
	} else if chainId == testnetId {
		fileName = "cmd/merlins/cmd/fury-test-5-assetlist-manual.json"
	} else {
		return nil, nil
	}
	...
	if err != nil {
		...
		if chainId == mainnetId || chainId == "" {
			fileName = "merlins-1-assetlist.json"
		} else if chainId == testnetId {
...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	encodingConfig := merlins.MakeEncodingConfig()
	...
	if err != nil {
		...
		homeDir = merlins.DefaultNodeHome
	}
...
func getHomeEnvironment() string {
	envPath := filepath.Join(merlins.DefaultNodeHome, ".env")
...
func initAppConfig() (string, interface{}) {
	type MerlinsMempoolConfig struct {
	...
	type CustomAppConfig struct {
		...
		MerlinsMempoolConfig MerlinsMempoolConfig `mapstructure:"merlins-mempool"`
	}
	...
	memCfg := MerlinsMempoolConfig{ArbitrageMinGasPrice: "0.01"}

	MerlinsAppCfg := CustomAppConfig{Config: *srvCfg, MerlinsMempoolConfig: memCfg}

	MerlinsAppTemplate := serverconfig.DefaultConfigTemplate + `
###############################################################################
###                      Merlins Mempool Configuration                      ###
###############################################################################
...
`
	...
	return MerlinsAppTemplate, MerlinsAppCfg
}
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig) {
	...
	rootCmd.AddCommand(
		// genutilcli.InitCmd(merlins.ModuleBasics, merlins.DefaultNodeHome),
		forceprune(),
		InitCmd(merlins.ModuleBasics, merlins.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, merlins.DefaultNodeHome),
		genutilcli.MigrateGenesisCmd(),
		ExportDeriveBalancesCmd(),
		StakedToCSVCmd(),
		AddGenesisAccountCmd(merlins.DefaultNodeHome),
		genutilcli.GenTxCmd(merlins.ModuleBasics, encodingConfig.TxConfig, banktypes.GenesisBalancesIterator{}, merlins.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(merlins.ModuleBasics),
		PrepareGenesisCmd(merlins.DefaultNodeHome, merlins.ModuleBasics),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(merlins.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		tmcmds.RollbackStateCmd,
		debugCmd,
		ConfigCmd(),
		ChangeEnvironmentCmd(),
		PrintEnvironmentCmd(),
		UpdateAssetListCmd(merlins.DefaultNodeHome, merlins.ModuleBasics),
	)

	server.AddCommands(rootCmd, merlins.DefaultNodeHome, newApp, createMerlinsAppAndExport, addModuleInitFlags)
	...
	rootCmd.AddCommand(
		...
		queryCommand(),
		txCommand(),
		keys.Commands(merlins.DefaultNodeHome),
	)
	// add rosetta
...
func queryCommand() *cobra.Command {
	...
	merlins.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	merlins.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")

	return cmd
}

// newApp initializes and returns a new Merlins app.
...
// createMerlinsAppAndExport creates and exports the new Merlins app, returns the state of the new Merlins app for a genesis file.
func createMerlinsAppAndExport(
	logger log.Logger, db dbm.DB, traceStore io.Writer, height int64, forZeroHeight bool, jailWhiteList []string,
	appOpts servertypes.AppOptions, modulesToExport []string,
) (servertypes.ExportedApp, error) {
	encCfg := merlins.MakeEncodingConfig() // Ideally, we would reuse the one created by NewRootCmd.
	encCfg.Marshaler = codec.NewProtoCodec(encCfg.InterfaceRegistry)
	loadLatest := height == -1
	homeDir := cast.ToString(appOpts.Get(flags.FlagHome))
	app := merlins.NewMerlinsApp(logger, db, traceStore, loadLatest, map[int64]bool{}, homeDir, 0, appOpts, merlins.EmptyWasmOpts)
...
Outputs:
	- cmd/merlins/cmd/merlins-1-assetlist-manual.json for merlins-1
	- cmd/merlins/cmd/fury-test-5-assetlist-manual.json for fury-test-5
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if args[0] == mainnetId || args[0] == "" {
				assetListURL = "https://raw.githubusercontent.com/merlins-labs/assetlists/main/merlins-1/merlins-1.assetlist.json"
				fileName = "cmd/merlins/cmd/merlins-1-assetlist-manual.json"
			} else if args[0] == testnetId {
				assetListURL = "https://raw.githubusercontent.com/merlins-labs/assetlists/main/fury-test-5/fury-test-5.assetlist.json"
				fileName = "cmd/merlins/cmd/fury-test-5-assetlist-manual.json"
			} else {
				return nil
			}
