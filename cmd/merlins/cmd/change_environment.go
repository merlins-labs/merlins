...
Example:
	merlins set-env mainnet
	merlins set-env testnet
	merlins set-env localnet [optional-chain-id]
	merlins set-env $HOME/.custom-dir
`,
		Args: customArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// dictates this as a requirement. If we changed the env file here, the merlins
...
Example:
	...
	Returns one of:
	- mainnet implying $HOME/.merlins
	- testnet implying $HOME/.merlins-test
	- localmerlins implying $HOME/.merlins-local
	- custom path`,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func environmentNameToPath(environmentName string) (string, error) {
	...
	case EnvTestnet:
		return filepath.Join(userHomeDir, ".merlins-test/"), nil
	case EnvLocalnet:
		return filepath.Join(userHomeDir, ".merlins-local/"), nil
	default:
...
func clientSettingsFromEnv(cmd *cobra.Command, environmentName, chainId string) error {
	envConfigs := map[string]map[string]string{
		EnvMainnet: {
			flags.FlagChainID:       "merlins-1",
			flags.FlagNode:          "https://rpc.merlins.zone:443",
			flags.FlagBroadcastMode: "block",
		},
		EnvTestnet: {
			flags.FlagChainID:       "fury-test-5",
			flags.FlagNode:          "https://rpc.testnet.merlins.zone:443",
			flags.FlagBroadcastMode: "block",
		},
		EnvLocalnet: {
			flags.FlagChainID:       "localmerlins",
			flags.FlagBroadcastMode: "block",
		},
	}
...
// .env file. This is used for the first time setup of the merlins home directory.
