...
func defaultClientConfig() *MerlinsCustomClient {
	return &MerlinsCustomClient{ChainID: chainID, KeyringBackend: keyringBackend, Output: output, Node: node, BroadcastMode: broadcastMode}
}
...
func writeConfigToFile(configFilePath string, config *MerlinsCustomClient) error {
	var buffer bytes.Buffer
	defaultMerlinsCustomClient := defaultClientConfig()
	...
	if config != nil {
		...
		defaultValue := reflect.ValueOf(defaultMerlinsCustomClient).Elem()
	...
	if err := configTemplate.Execute(&buffer, defaultMerlinsCustomClient); err != nil {
		return err
	}
...
func getClientConfig(configPath string, v *viper.Viper) (*MerlinsCustomClient, error) {
...
func SetCustomEnvVariablesFromClientToml(ctx client.Context) {
	...
	// gas
	setEnvFromConfig("gas", "MERLINS_GAS")
	setEnvFromConfig("gas-prices", "MERLINS_GAS_PRICES")
	setEnvFromConfig("gas-adjustment", "MERLINS_GAS_ADJUSTMENT")
	// fees
	setEnvFromConfig("fees", "MERLINS_FEES")
	// human readable denoms
	setEnvFromConfig("human-readable-denoms-input", "MERLINS_HUMAN_READABLE_DENOMS_INPUT")
	setEnvFromConfig("human-readable-denoms-output", "MERLINS_HUMAN_READABLE_DENOMS_OUTPUT")
}
...
func GetHumanReadableDenomEnvVariables() (bool, bool) {
	humanReadableDenomsInputStr := os.Getenv("MERLINS_HUMAN_READABLE_DENOMS_INPUT")
	humanReadableDenomsInput, err := strconv.ParseBool(humanReadableDenomsInputStr)
	if err != nil {
		return false, false
	}
	humanReadableDenomsOutputStr := os.Getenv("MERLINS_HUMAN_READABLE_DENOMS_OUTPUT")
