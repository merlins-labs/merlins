...
PREV_MERLINS_DEV_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/fanfury/merlins-dev/tags?page=1&page_size=100'            | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")
PREV_MERLINS_E2E_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/fanfury/merlins-e2e-init-chain/tags?page=1&page_size=100' | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")
