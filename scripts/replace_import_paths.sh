...
done < <(find . -name go.mod -exec grep -l "github.com/merlins-labs/merlins/v17" {} \; | grep -v  "^./go.mod$" | sed 's|/go.mod||' | sed 's|^./||')

replace_paths() {
    file="${1}"
    sed -i "s/github.com\/merlins-labs\/merlins\/v${version_to_replace}/github.com\/merlins-labs\/merlins\/v${NEXT_MAJOR_VERSION}/g" ${file}
}
