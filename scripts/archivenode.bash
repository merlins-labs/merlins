...
# 2nd NB: you can change MERLINS_PRUNING=nothing to MERLINS_PRUNING=default OR you could also set the pruning settings manually with MERLINS_PRUNING=custom
...
export MERLINS_PRUNING=nothing
export MERLINS_DB_BACKEND=pebbledb
export MERLINS_P2P_MAX_NUM_OUTBOUND_PEERS=500
export MERLINS_P2P_MAX_NUM_INBOUND_PEERS=500
export MERLINS_P2P_SEEDS=$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/merlins/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')
export MERLINS_P2P_LADDR=tcp://0.0.0.0:2001
...
merlins init speedrun
wget -O ~/.merlins/config/genesis.json https://github.com/merlins-labs/networks/raw/main/merlins-1/genesis.json
merlins start --db_backend pebbledb
git reset --hard
...
go mod edit -replace github.com/tendermint/tm-db=github.com/notional-labs/tm-db@136c7b6
go mod tidy
go install -ldflags '-w -s -X github.com/cosmos/cosmos-sdk/types.DBBackend=pebbledb' -tags pebbledb ./...time merlins start --db_backend pebbledb
merlins start --db_backend pebbledb
git reset --hard
