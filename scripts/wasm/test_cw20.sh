...
merlins keys add demo --keyring-backend test
VAL=$(merlins keys show -a validator --keyring-backend test)
DEMO=$(merlins keys show -a demo --keyring-backend test)
...
merlins tx wasm instantiate $CODE "$INIT" --label "First Coin" --no-admin --from validator \
...
CONTRACT=fury14hj2tavq8fpesdwxxcu44rty3hh90vhujrvcmstl4zr3txmfvw9sq2r9g9
...
merlins query wasm contract-state smart $CONTRACT "$QUERY"
echo "Demo Balance:"
merlins query wasm contract-state smart $CONTRACT "$QUERY_DEMO"
...
merlins tx wasm execute $CONTRACT "$TRANSFER" --from validator --keyring-backend test \
