#!/bin/sh
MERLINS_HOME="/tmp/merlins$(date +%s)"
...
if [ $LEN_GENTX -eq 0 ]; then
    ...
    make build
    chmod +x ./build/merlins

    ./build/merlins keys add $RANDOM_KEY --keyring-backend test --home $MERLINS_HOME

    ./build/merlins init --chain-id $CHAIN_ID validator --home $MERLINS_HOME
    ...
    ./build/merlins add-genesis-account $RANDOM_KEY 100000000000000$DENOM --home $MERLINS_HOME \
        --keyring-backend test

    ./build/merlins gentx $RANDOM_KEY 90000000000000$DENOM --home $MERLINS_HOME \
        --keyring-backend test --chain-id $CHAIN_ID

    cp ../$GENTX_FILE $MERLINS_HOME/config/gentx/

    echo "..........Collecting gentxs......."
    ./build/merlins collect-gentxs --home $MERLINS_HOME
    sed -i '/persistent_peers =/c\persistent_peers = ""' $MERLINS_HOME/config/config.toml

    ./build/merlins validate-genesis --home $MERLINS_HOME

    echo "..........Starting node......."
    ./build/merlins start --home $MERLINS_HOME &

    sleep 1800s

    echo "...checking network status.."

    ./build/merlins status --node http://localhost:26657

    echo "...Cleaning the stuff..."
    killall merlins >/dev/null 2>&1
