...
func SetupTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	merlinsApp := app.Setup(false)
	return merlinsApp, app.NewDefaultGenesisState()
}
...
func (chain *TestChain) MoveEpochsToTheFuture() error {
	epochsKeeper := chain.GetMerlinsApp().EpochsKeeper
...
// GetMerlinsApp returns the current chain's app as an MerlinsApp
func (chain *TestChain) GetMerlinsApp() *app.MerlinsApp {
	v, _ := chain.App.(*app.MerlinsApp)
	return v
}
