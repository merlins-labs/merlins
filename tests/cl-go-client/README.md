...
```bash
make set-env localmerlins # sets environment to $HOME/.merlins-local
...
Configure a different `merlins` environment with configs.

```bash
make set-env .merlins-testnet-script

merlins init test-script

cd $HOME/.merlins-testnet-script/config
...
Next, manually edit the `localmerlinsFromHomePath` variable in the script:
<https://github.com/merlins-labs/merlins/blob/98025f185ab2ee1b060511ed22679112abcc08fa/tests/cl-go-client/main.go#L28>

Set the value to `.merlins-testnet-script` and save.
