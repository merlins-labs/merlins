...
then
    ...
    echo $MNEMONIC | merlins init -o --chain-id=$CHAIN_ID --home $MERLINS_HOME --recover $MONIKER 2> /dev/null
    echo $MNEMONIC | merlins keys add my-key --recover --keyring-backend test > /dev/null 2>&1

    ACCOUNT_PUBKEY=$(merlins keys show --keyring-backend test my-key --pubkey | dasel -r json '.key' --plain)
    ACCOUNT_ADDRESS=$(merlins keys show -a --keyring-backend test my-key --bech acc)

    VALIDATOR_PUBKEY_JSON=$(merlins tendermint show-validator --home $MERLINS_HOME)
    VALIDATOR_PUBKEY=$(echo $VALIDATOR_PUBKEY_JSON | dasel -r json '.key' --plain)
    VALIDATOR_HEX_ADDRESS=$(merlins debug pubkey $VALIDATOR_PUBKEY_JSON 2>&1 --home $MERLINS_HOME | grep Address | cut -d " " -f 2)
    VALIDATOR_ACCOUNT_ADDRESS=$(merlins debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MERLINS_HOME | grep Acc | cut -d " " -f 3)
    VALIDATOR_OPERATOR_ADDRESS=$(merlins debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MERLINS_HOME | grep Val | cut -d " " -f 3)
    VALIDATOR_CONSENSUS_ADDRESS=$(merlins debug bech32-convert $VALIDATOR_OPERATOR_ADDRESS -p furyvalcons  --home $MERLINS_HOME 2>&1)

    python3 -u testnetify.py \
    -i /merlins/state_export.json \
...
merlins start --home $MERLINS_HOME --x-crisis-skip-assert-invariants
