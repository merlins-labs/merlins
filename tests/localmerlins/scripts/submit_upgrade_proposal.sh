...
MERLINS_HOME=$HOME/.merlins-local/
...
make_proposal() {
    ...
    MERLINS_CMD="merlins tx gov submit-proposal software-upgrade \
        $UPGRADE_VERSION \
    ...
    PROPOSAL_JSON=$(eval "$MERLINS_CMD")
...
# Query proposal
query_proposal() {
    merlins q gov proposal $PROPOSAL_ID \
        --node $RPC_NODE \
        -o json | jq
}
...
vote_on_proposal() {
    ...
    MERLINS_CMD="merlins tx gov vote $PROPOSAL_ID yes \
        --from $KEY \
        --chain-id $CHAIN_ID \
        --fees $TX_FEES \
        --node $RPC_NODE \
