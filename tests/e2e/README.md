...
depending on the Merlins codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
Merlins version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older Merlins version to the container,
...
1. Chain Initialization
    ...
    of the previous Merlins version, inside `configurer/upgrade.go`. This is
    ...
    `UpgradeConfigurer` takes older merlins versions and configures chains from them. 
    ...
    current branch version of Merlins codebase
    ...
    with IBC initialized at the previous Merlins version
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older Merlins version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch Merlins version is spun up to continue with testing
    ...
    [Example](https://github.com/merlins-labs/merlins/blob/c5d5c9f0c6b5c7fdf9688057eb78ec793f6dd580/tests/e2e/configurer/configurer.go#L166)
...
This package directly depends on the Merlins codebase.
...
production merlins container to start the `merlins` service.
...
### To build the debug Merlins image
...
- `MERLINS_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If MERLINS_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `MERLINS_E2E_SKIP_IBC` - when true, skips the IBC tests.

- `MERLINS_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `MERLINS_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `MERLINS_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `MERLINS_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `MERLINS_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
    ...
    "configurations": [
        {
            ...
            "env": {
                "MERLINS_E2E": "True",
                "MERLINS_E2E_SKIP_IBC": "true",
                "MERLINS_E2E_SKIP_UPGRADE": "true",
                "MERLINS_E2E_SKIP_CLEANUP": "true",
                "MERLINS_E2E_SKIP_STATE_SYNC": "true",
                "MERLINS_E2E_UPGRADE_VERSION": "v13",
                "MERLINS_E2E_DEBUG_LOG": "true",
            },
...
1. Order of executions of tests in `e2e_test.go` is alphabetic.
    ...
    A way to deal with this problem: disable upgrade logic by setting `MERLINS_E2E_SKIP_UPGRADE` to `false` and see how test performs.
...
We set the following parameters Hermes configs to enable the consensus min fee in Merlins:
    ...
    In Merlins, we set consensus min fee = .0025 ufury / gas * 400000 gas = 1000
...
This section contains information about debugging merlins's `e2e` tests.
...
1. Executing commands on docker containers
    ...
    Example: `docker exec fury-test-a-node-prune-nothing-snapshot merlins status` will print a node status of `fury-test-a-node-prune-nothing-snapshot` container. 
...
3. Viewing docker container logs when run merlins's `e2e` tests in CI
