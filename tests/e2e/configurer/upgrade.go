...
import (
	...
	"github.com/merlins-labs/merlins/v17/tests/e2e/containers"
...
func (uc *UpgradeConfigurer) ConfigureChain(chainConfig *chain.Config) error {
	...
	tmpDir, err := os.MkdirTemp("", "merlins-e2e-testnet-")
	if err != nil {
		return err
	}
...
func (uc *UpgradeConfigurer) upgradeContainers(chainConfig *chain.Config, propHeight int64) error {
	...
	uc.containerManager.MerlinsRepository = containers.CurrentBranchFuryRepository
	uc.containerManager.MerlinsTag = containers.CurrentBranchFuryTag
