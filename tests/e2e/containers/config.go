...
const (
	...
	CurrentBranchOsmoRepository = "merlins"
	...
	previousVersionOsmoRepository = "furylabs/merlins-dev"
...
func NewImageConfig(isUpgrade, isFork bool) ImageConfig {
	...
	if !isUpgrade {
		...
		// the need for Docker.
		config.MerlinsRepository = CurrentBranchOsmoRepository
		config.MerlinsTag = CurrentBranchOsmoTag
		return config
	}
	...
	if isFork {
		...
		config.MerlinsRepository = previousVersionOsmoRepository
		config.MerlinsTag = previousVersionOsmoTag
	}
