...
func NewManager(isUpgrade bool, isFork bool, isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("merlins-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) RunNodeResource(chainId string, containerName, valCondifDir string) (*dockertest.Resource, error) {
	...
	runOpts := &dockertest.RunOptions{
		Name:       containerName,
		Repository: m.MerlinsRepository,
		Tag:        m.MerlinsTag,
		NetworkID:  m.network.Network.ID,
		User:       "root:root",
		Cmd:        []string{"start"},
		Mounts: []string{
			fmt.Sprintf("%s/:/merlins/.merlins", valCondifDir),
			fmt.Sprintf("%s/scripts:/merlins", pwd),
		},
	}
