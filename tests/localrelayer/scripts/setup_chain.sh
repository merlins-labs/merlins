...
add_genesis_accounts () {
    ...
    echo $VALIDATOR_MNEMONIC | merlins keys add $VALIDATOR_MONIKER --recover --keyring-backend=test --home $MERLINS_HOME
    VALIDATOR_ACCOUNT=$(merlins keys show -a $VALIDATOR_MONIKER --keyring-backend test --home $MERLINS_HOME)
    merlins add-genesis-account $VALIDATOR_ACCOUNT 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    
    # Faucet
    echo "🚰 Add faucet account"
    echo $FAUCET_MNEMONIC | merlins keys add faucet --recover --keyring-backend=test --home $MERLINS_HOME
    FAUCET_ACCOUNT=$(merlins keys show -a faucet --keyring-backend test --home $MERLINS_HOME)
    merlins add-genesis-account $FAUCET_ACCOUNT 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME

    # Relayer
    echo "🔗 Add relayer account"
    echo $RELAYER_MNEMONIC | merlins keys add relayer --recover --keyring-backend=test --home $MERLINS_HOME
    RELAYER_ACCOUNT=$(merlins keys show -a relayer --keyring-backend test --home $MERLINS_HOME)
    merlins add-genesis-account $RELAYER_ACCOUNT 1000000000ufury,1000000000uion,1000000000stake --home $MERLINS_HOME
    
    merlins gentx $VALIDATOR_MONIKER 500000000ufury --keyring-backend=test --chain-id=$CHAIN_ID --home $MERLINS_HOME
...
then
    ...
    echo $VALIDATOR_MNEMONIC | merlins init -o --chain-id=$CHAIN_ID --home $MERLINS_HOME --recover $VALIDATOR_MONIKER
    edit_genesis
    add_genesis_accounts
    edit_config
fi
