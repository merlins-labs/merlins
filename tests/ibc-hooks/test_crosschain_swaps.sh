...
alias chainA="merlins --node http://localhost:26657 --chain-id localmerlins-a"
alias chainB="merlins --node http://localhost:36657 --chain-id localmerlins-b"

# setup the keys
echo "bottom loan skill merry east cradle onion journey palm apology verb edit desert impose absurd oil bubble sweet glove shallow size build burst effort" | merlins --keyring-backend test keys add validator --recover || echo "key exists"
echo "increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great" | merlins --keyring-backend test keys add faucet --recover || echo "key exists"

export VALIDATOR=$(merlins keys show validator -a)
...
chainB tx wasm store ./bytecode/crosschain_swaps.wasm --from validator --gas auto --gas-prices 0.1ufury --gas-adjustment 1.3 -y -b block
...
MEMO=$(jenv -c '{"wasm": {"contract": $CROSSCHAIN_SWAPS_ADDRESS, "msg": {"merlins_swap":{"swap_amount":"100","output_denom":"ufury","slippage":{"twap": {"slippage_percentage":"20", "window_seconds": 10}},"receiver":$VALIDATOR, "on_failed_delivery": "do_nothing"}}}}')
