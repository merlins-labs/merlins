...
func (suite *HooksTestSuite) forceContractToEmitAckForPacket(merlinsApp *app.MerlinsApp, ctx sdk.Context, contractAddr sdk.AccAddress, packet channeltypes.Packet, success bool) ([]byte, error) {
...
func (suite *HooksTestSuite) TestWasmHooksAsyncAcks() {
	...
	allAcks := merlinsApp.IBCKeeper.ChannelKeeper.GetAllPacketAcks(suite.chainA.GetContext())
	...
	_, err = suite.forceContractToEmitAckForPacket(merlinsApp, suite.chainA.GetContext(), contractAddr, alreadyAckedPacket, true)
	...
	merlinsApp.IBCHooksKeeper.SetParams(suite.chainA.GetContext(), params)
	...
	for _, tc := range testCases {
		...
		_, err = suite.forceContractToEmitAckForPacket(merlinsApp, suite.chainA.GetContext(), contractAddr2, packet, tc.success)
		...
		_, err = suite.forceContractToEmitAckForPacket(merlinsApp, ctx, contractAddr, packet, tc.success)
		totalExpectedAcks++
		suite.Require().NoError(err)
