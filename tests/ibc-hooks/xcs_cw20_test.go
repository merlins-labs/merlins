...
func (suite *HooksTestSuite) TransferCW20Tokens(path *ibctesting.Path, cw20Addr, cw20ics20Addr, receiver sdk.AccAddress, amount, memo string) (*sdk.Result, []byte) {
	...
	merlinsApp := chainB.GetMerlinsApp()
...
func (suite *HooksTestSuite) setupCW20PoolAndRoutes(chain *merlinsibctesting.TestChain, swaprouterAddr sdk.AccAddress, cw20IbcDenom string, amount sdk.Int) {
	merlinsAppA := chain.GetMerlinsApp()
...
func (suite *HooksTestSuite) getCW20Balance(chain *merlinsibctesting.TestChain, cw20Addr, addr sdk.AccAddress) sdk.Int {
...
func (suite *HooksTestSuite) TestCW20ICS20() {
	...
	merlinsAppB := chainB.GetMerlinsApp()
	...
	balanceStakeReceiver := merlinsAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), stakeAB)
	suite.Require().Equal(int64(0), balanceStakeReceiver.Amount.Int64())

	// Transfer the tokens with a memo for XCS
	swapMsg := fmt.Sprintf(`{"merlins_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
	...
	swapMsg = fmt.Sprintf(`{"merlins_swap":{"output_denom":"%s","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"chainB-cw20/%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		cw20IbcDenom,
		receiver2,
	)
	xcsMsg = fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
	...
	balanceStakeReceiver2 := merlinsAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), stakeAB)
