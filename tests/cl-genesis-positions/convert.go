...
func ConvertSubgraphToMerlinsGenesis(positionCreatorAddresses []sdk.AccAddress, subgraphFilePath string) (*clgenesis.GenesisState, *banktypes.GenesisState) {
	...
	_, _, _, _, err = merlins.App.ConcentratedLiquidityKeeper.CreateFullRangePosition(merlins.Ctx, pool.GetId(), merlins.TestAccs[0], sdk.NewCoins(sdk.NewCoin(msgCreatePool.Denom0, sdk.NewInt(100)), sdk.NewCoin(msgCreatePool.Denom1, sdk.NewInt(100))))
	if err != nil {
		panic(err)
	}
