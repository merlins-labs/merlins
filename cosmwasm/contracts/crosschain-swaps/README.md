...
Note that in this example we are using FURY stored in juno, so the token we send is 
...
#### Prerequisites
* merlins command line tool installed and configured.
...
```bash
VALIDATOR=$(merlins keys show validator -a --keyring-backend test)
...
```bash
echo "bottom loan skill merry east cradle onion journey palm apology verb edit desert impose absurd oil bubble sweet glove shallow size build burst effort" | merlins --keyring-backend test keys add validator --recover
echo "increase bread alpha rigid glide amused approve oblige print asset idea enact lawn proof unfold jeans rabbit audit return chuckle valve rather cactus great" | merlins --keyring-backend test  keys add faucet --recover
```
