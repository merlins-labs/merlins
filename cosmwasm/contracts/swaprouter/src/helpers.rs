...
use merlins_std::shim::Timestamp as MerlinsTimestamp;
use merlins_std::types::merlins::gamm::v1beta1::{
    MsgSwapExactAmountIn, QueryTotalPoolLiquidityRequest, SwapAmountInRoute,
};
use merlins_std::types::merlins::twap::v1beta1::TwapQuerier;
...
pub fn calculate_min_output_from_twap(
    ...
    //  price of <out> is X<in> (i.e.: price of atom is Xfury)
    ...
    let start_time = MerlinsTimestamp {
        seconds: start_time.seconds() as i64,
        nanos: 0_i32,
    };

    let end_time = MerlinsTimestamp {
