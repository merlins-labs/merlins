...
mod tests {
    ...
    fn test_set_contract_alias() {
        ...
        let address = "fury12smx2wdlyttvyzvzg54y2vnqwq2qjatel8rck9".to_string();
        ...
        assert_eq!(
            CONTRACT_ALIAS_MAP
                ...
                .unwrap(),
            "fury12smx2wdlyttvyzvzg54y2vnqwq2qjatel8rck9"
        );
        ...
        let msg = ExecuteMsg::ModifyContractAlias {
            operations: vec![ContractAliasInput {
                operation: Operation::Set,
                alias: alias.clone(),
                address: Some("fury1fsdaf7dsfasndjklk3jndskajnfkdjsfjn3jka".to_string()),
                new_alias: None,
            }],
        };
        ...
        let msg = ExecuteMsg::ModifyContractAlias {
            operations: vec![ContractAliasInput {
                ...
                address: Some("fury1nna7k5lywn99cd63elcfqm6p8c5c4qcuqwwflx".to_string()),
                new_alias: None,
            }],
        };
    ...
    fn test_set_chain_channel_link() {
        ...
        // Set the canonical channel link between merlins and cosmos to channel-0
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Set,
                source_chain: "MERLINS".to_string(),
        ...
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("merlins", "cosmos"))
                .unwrap(),
            ("channel-0", true).into()
        );

        // Verify that channel-0 on merlins is linked to cosmos
        assert_eq!(
            CHANNEL_ON_CHAIN_CHAIN_MAP
                .load(&deps.storage, ("channel-0", "merlins"))
                .unwrap(),
            ("cosmos", true).into()
        );

        // Attempt to set the canonical channel link between merlins and cosmos to channel-150
        ...
        // Attempt to set the canonical channel link between mars and merlins to channel-1 with an unauthorized address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Set,
                source_chain: "mars".to_string(),
                destination_chain: "merlins".to_string(),
        ...
        let expected_error = ContractError::Unauthorized {};
        assert_eq!(result.unwrap_err(), expected_error);
        assert!(!CHAIN_TO_CHAIN_CHANNEL_MAP.has(&deps.storage, ("mars", "merlins")));

        // Set the canonical channel link between mars and merlins to channel-1 with a mars chain admin address
        let chain_admin_info = mock_info(CHAIN_ADMIN, &[]);
        contract::execute(deps.as_mut(), mock_env(), chain_admin_info.clone(), msg).unwrap();
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("mars", "merlins"))
                .unwrap(),
            ("channel-1", true).into()
        );
        assert_eq!(
    ...
    fn test_change_chain_channel_link() {
        ...
        // Change the canonical channel link between merlins and cosmos to channel-150 with the global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
        ...
        // Verify that the channel between merlins and cosmos has changed from channel-0 to channel-150
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("merlins", "cosmos"))
                .unwrap(),
        ...
        // Change channel-0 link of merlins from cosmos to regen with the global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Change,
                source_chain: "merlins".to_string(),
        ...
        // Verify that channel-150 on merlins is linked to regen
        assert_eq!(
            CHANNEL_ON_CHAIN_CHAIN_MAP
                .load(&deps.storage, ("channel-150", "merlins"))
                .unwrap(),
            ("regen", true).into()
        );

        // Attempt to change the canonical channel link between merlins and regen to channel-2 with an unauthorized address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Change,
                source_chain: "merlins".to_string(),
        ...
        // Set the canonical channel link between mars and merlins to channel-1 with a chain admin address
        let info_chain_admin = mock_info(CHAIN_ADMIN, &[]);
        contract::execute(deps.as_mut(), mock_env(), info_chain_admin, msg).unwrap();
        assert_eq!(
            CHAIN_TO_CHAIN_CHANNEL_MAP
                .load(&deps.storage, ("merlins", "regen"))
                .unwrap(),
            ("channel-2", true).into()
        );
        ...
        // Attempt to update a merlins channel link with a merlins chain maintainer address
    ...
    fn test_remove_chain_channel_link() {
        ...
        // Remove the merlins cosmos link with a global admin address
        let msg = ExecuteMsg::ModifyChainChannelLinks {
            operations: vec![ConnectionInput {
                operation: FullOperation::Remove,
                source_chain: "merlins".to_string(),
        ...
        assert!(!CHAIN_TO_CHAIN_CHANNEL_MAP.has(&deps.storage, ("merlins", "cosmos")));
        ...
        // Attempt to remove the merlins regen link with a merlins chain maintainer address
