...
func NewCmdUpdateUnpoolWhitelistProposal() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "merlins tx gov submit-proposal update-unpool-whitelist --pool-ids \"1, 2, 3\" --title \"Title\" --description \"Description\"",
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
...
func NewCreateFullRangePositionAndSuperfluidDelegateCmd() (*osmocli.TxCliDesc, *types.MsgCreateFullRangePositionAndSuperfluidDelegate) {
	return &osmocli.TxCliDesc{
		...
		Example: "create-full-range-position-and-sf-delegate 100000000ufury,10000udai 45 --from val --chain-id merlins-1",
...
func NewUnbondConvertAndStake() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "unbond-convert-and-stake 10 fury1xxx 100000ufury",
