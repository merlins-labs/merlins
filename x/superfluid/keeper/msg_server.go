...
// Note that the amount of delegation is not equal to the equivalent amount of fury within the lock.
// Instead, we use the fury equivalent multiplier stored in the latest epoch, calculate how much
// fury equivalent is in lock, and use the risk adjusted fury value. The minimum risk ratio works as a parameter
