	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			for _, lock := range locks {
				...
				furyAmount, err := s.App.SuperfluidKeeper.GetSuperfluidFURYTokens(s.Ctx, intermediaryAcc.Denom, tc.unlockAmount)
				s.Require().NoError(err)
				...
				s.Require().True(furyAmount.IsPositive())
				supplyAfter := s.App.BankKeeper.GetSupply(s.Ctx, bondDenom)
				s.Require().Equal(supplyAfter, supplyBefore.Sub(sdk.NewCoin(bondDenom, furyAmount)))
...
func (s *KeeperTestSuite) TestConvertGammSharesToFuryAndStake() {
	...
	testCases := map[string]tc{
		...
		"error: invalid val address": {
			...
			expectedError:     "invalid Bech32 prefix; expected furyvaloper, got fury",
		},
...
func (s *KeeperTestSuite) getExpectedBondDenomPoolAmtAfterConvert(sender sdk.AccAddress, poolId uint64, sharesToStake sdk.Coin) sdk.Int {
	...
	_, err = s.App.PoolManagerKeeper.SwapExactAmountIn(cc, sender, poolId, nonFuryCoin, bondDenom, sdk.ZeroInt())
	s.Require().NoError(err)
