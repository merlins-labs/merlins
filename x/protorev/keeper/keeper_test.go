...
func (s *KeeperTestSuite) SetupTest() {
	...
	baseDenomPriorities := []types.BaseDenom{
		{
			Denom:    types.MerlinsDenomination,
	...
	s.balances = sdk.NewCoins(
		sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(9000000000000000000)),
...
// akash <-> types.MerlinsDenomination
// juno <-> types.MerlinsDenomination
// ethereum <-> types.MerlinsDenomination
// bitcoin <-> types.MerlinsDenomination
// canto <-> types.MerlinsDenomination
// and so on....
func (s *KeeperTestSuite) setUpPools() {
	...
	s.pools = []Pool{
		...
		{ // Pool 6
			PoolAssets: []balancer.PoolAsset{
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(1000)),
					Weight: sdk.NewInt(1),
				},
				{
					Token:  sdk.NewCoin("Atom", sdk.NewInt(1000)),
		...
		{ // Pool 22
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(191801648570)),
					Weight: sdk.NewInt(1),
				},
			},
		...
		{ // Pool 24
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(13901565323)),
					Weight: sdk.NewInt(1),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
		...
		{ // Pool 25
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(139015653231902)),
					Weight: sdk.NewInt(1),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
		...
		{ // Pool 26
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(171274446980)),
					Weight: sdk.NewInt(50),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
		...
		{ // Pool 27
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(13466662920841)),
					Weight: sdk.NewInt(50),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
		...
		{ // Pool 35
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(1694086377216)),
					Weight: sdk.NewInt(1),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
		...
		{ // Pool 36
			PoolAssets: []balancer.PoolAsset{
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(2774812791932)),
					Weight: sdk.NewInt(1),
				},
				{
		...
		{ // Pool 38
			PoolAssets: []balancer.PoolAsset{
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(6111815027)),
					Weight: sdk.NewInt(1),
				},
				{
		...
		{ // Pool 39
			PoolAssets: []balancer.PoolAsset{
				...
				{
					Token:  sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(17000185817963)),
					Weight: sdk.NewInt(1),
				},
			},
			SpreadFactor: sdk.NewDecWithPrec(2, 3),
	...
	s.stableSwapPools = []StableSwapPool{
		...
		{ // Pool 49 - Used for CL testing
			initialLiquidity: sdk.NewCoins(
				sdk.NewCoin("ufury", sdk.NewInt(10_000_000_000_000)),
	...
	// Pool 52
	// Create the CL pool
	clPool := s.PrepareCustomConcentratedPool(s.TestAccs[0], "epochTwo", "ufury", apptesting.DefaultTickSpacing, sdk.ZeroDec())
	fundCoins := sdk.NewCoins(sdk.NewCoin("epochTwo", sdk.NewInt(10_000_000_000_000)), sdk.NewCoin("ufury", sdk.NewInt(10_000_000_000_000)))
	...
	// Create the CL pool
	clPool = s.PrepareCustomConcentratedPool(s.TestAccs[0], "epochTwo", "ufury", apptesting.DefaultTickSpacing, sdk.ZeroDec())
	fundCoins = sdk.NewCoins(sdk.NewCoin("epochTwo", sdk.NewInt(2_000_000_000)), sdk.NewCoin("ufury", sdk.NewInt(1_000_000_000)))
	s.FundAcc(s.TestAccs[0], fundCoins)
	s.CreateFullRangePosition(clPool, fundCoins)
...
func (s *KeeperTestSuite) CreateCLPoolAndArbRouteWith_28000_Ticks() {
	// Create the CL pool
	clPool := s.PrepareCustomConcentratedPool(s.TestAccs[2], "ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", "ufury", 100, sdk.NewDecWithPrec(2, 3))
	fundCoins := sdk.NewCoins(sdk.NewCoin("ufury", sdk.NewInt(1000000000000000000)), sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", sdk.NewInt(1000000000000000000)))
	s.FundAcc(s.TestAccs[2], fundCoins)

	// Create 28000 ticks in the CL pool, 14000 on each side
	tokensProvided := sdk.NewCoins(sdk.NewCoin("ufury", sdk.NewInt(100000)), sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", sdk.NewInt(100000)))
...
func (s *KeeperTestSuite) setUpTokenPairRoutes() {
	...
	// Stableswap Route
	ufuryUSDC := types.NewTrade(0, types.MerlinsDenomination, "usdc")
	usdcBUSD := types.NewTrade(40, "usdc", "busd")
	busdUFURY := types.NewTrade(30, "busd", types.MerlinsDenomination)
	...
	fourPool1 := types.NewTrade(35, "test/1", types.MerlinsDenomination)
	fourPool2 := types.NewTrade(36, types.MerlinsDenomination, "test/2")
	fourPool3 := types.NewTrade(0, "test/2", "Atom")

	// Two-Pool Route
	twoPool0 := types.NewTrade(0, "ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", types.MerlinsDenomination)
	twoPool1 := types.NewTrade(39, types.MerlinsDenomination, "ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7")

	// Doomsday Route - Stableswap
	doomsdayStable0 := types.NewTrade(29, types.MerlinsDenomination, "usdc")
	doomsdayStable1 := types.NewTrade(0, "usdc", "busd")
	doomsdayStable2 := types.NewTrade(30, "busd", types.MerlinsDenomination)
	...
	s.tokenPairArbRoutes = []types.TokenPairArbRoutes{
		...
		{
			TokenIn:  "usdc",
			TokenOut: types.MerlinsDenomination,
			ArbRoutes: []types.Route{
				{
					StepSize: standardStepSize,
		...
		{
			TokenIn:  types.MerlinsDenomination,
			TokenOut: "ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7",
			ArbRoutes: []types.Route{
				{
					StepSize: standardStepSize,
