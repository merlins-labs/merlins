...
func (k Keeper) ConvertProfits(ctx sdk.Context, inputCoin sdk.Coin, profit sdk.Int) (sdk.Int, error) {
	if inputCoin.Denom == types.MerlinsDenomination {
		return profit, nil
	}

	// Get highest liquidity pool ID for the input coin and ufury
	conversionPoolID, err := k.GetPoolForDenomPair(ctx, types.MerlinsDenomination, inputCoin.Denom)
	if err != nil {
		return profit, err
	}
