...
func (s *KeeperTestSuite) TestGetAllBaseDenoms() {
	...
	s.Require().Equal(baseDenoms[0].Denom, types.MerlinsDenomination)
...
func (s *KeeperTestSuite) TestGetPoolForDenomPair() {
	...
	s.App.ProtoRevKeeper.SetPoolForDenomPair(s.Ctx, "Atom", types.MerlinsDenomination, 1000)
	pool, err := s.App.ProtoRevKeeper.GetPoolForDenomPair(s.Ctx, "Atom", types.MerlinsDenomination)
	s.Require().NoError(err)
	...
	s.App.ProtoRevKeeper.SetPoolForDenomPair(s.Ctx, types.MerlinsDenomination, "Atom", 3000)
	pool, err = s.App.ProtoRevKeeper.GetPoolForDenomPair(s.Ctx, types.MerlinsDenomination, "Atom")
	s.Require().NoError(err)
	...
	_, err = s.App.ProtoRevKeeper.GetPoolForDenomPair(s.Ctx, "Atom", types.MerlinsDenomination)
	s.Require().Error(err)
...
func (s *KeeperTestSuite) TestGetDeveloperFees() {
	...
	furyFees, err := s.App.ProtoRevKeeper.GetDeveloperFees(s.Ctx, types.MerlinsDenomination)
	s.Require().Error(err)
	...
	err = s.App.ProtoRevKeeper.SetDeveloperFees(s.Ctx, sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(100)))
	s.Require().NoError(err)
	...
	furyFees, err = s.App.ProtoRevKeeper.GetDeveloperFees(s.Ctx, types.MerlinsDenomination)
	s.Require().NoError(err)
	s.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(100)), furyFees)
	atomFees, err = s.App.ProtoRevKeeper.GetDeveloperFees(s.Ctx, "Atom")
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestDeleteDeveloperFees() {
	...
	s.App.ProtoRevKeeper.DeleteDeveloperFees(s.Ctx, types.MerlinsDenomination)
