...
9. Redistributes the profit captured back to the Merlins ecosystem based on Governance.

For ecosystem context about the purpose of the module, please see the ProtoRev governance proposal discussion: [https://gov.merlins.zone/discussion/7078-skip-x-merlins-proposal-to-capture-mev-as-protocol-revenue-on-chain](https://gov.merlins.zone/discussion/7078-skip-x-merlins-proposal-to-capture-mev-as-protocol-revenue-on-chain)
...
DenomPairToPool takes in a base denomination (read below) – denom that is used to build routes (ex. fury, atom, usdc) – and a denom to match (akash, juno) and returns the highest liquidity pool id between the pair of denominations. For example, an input might look like (fury, juno) —> poolID: 5. This store is directly tied to the highest liquidity method (described in state transitions below). Each base denomination is going to have its own set of denominations it maps to.
...
The developer account is set through a MsgSetDeveloperAccount tx. This is the account that will be able to withdraw a portion of the profits from `x/protorev` as specified by the Merlins ↔ Skip proposal. Only the admin account has permission to make this message.
...
**Highest Liquidity Pools:** Updated via the daily epoch, the module iterates through all the pools and stores the highest liquidity pool for every asset that pairs with any of the base denominations the module stores (for example, the fury/juno key will have a single pool id stored, that pool id having the most liquidity out of all the fury/juno pools). New base denominations can be added or removed on an as needed basis by the admin account. A base denomination is just another way of describing the denomination we want to use for cyclic arbitrage. This store is then used to create routes at runtime after analyzing a swap. This store is updated through the `epoch` hook and when the admin account submits a `MsgSetBaseDenoms` tx.
...
- Merlins —> Akash (on pool 1), Akash —> Juno (on pool 4), Juno —> Merlins (on pool 2)

It does so by finding the highest liquidity pool between (Merlins, Akash) —> pool 1 and the highest liquidity pool between (Merlins, Juno) —> pool 2. If there is no highest liquidity pool pair between (Merlins, Juno) or (Merlins, Akash), no route will be generated.
...
Populated through the admin account, the module’s keeper holds a KV store that associates token pairs (for example, fury/juno) to the routes that result in a high percentage of arbitrage profit on Merlins (as determined by external analysis).
...
- The admin is not a valid bech32 address
- The signature of the user does not match the admin account’s
- Merlins is not the first base denom in the list
...
Find below a lost of `merlins` commands added with the `x/protorev` module. A CLI command can look like this:

```bash
merlins query protorev params
```
...
| --- | --- | --- |
| gRPC | merlins.protorev.Query/Params | Queries the parameters of the module |
| gRPC | merlins.protorev.Query/GetProtoRevNumberOfTrades | Queries the number of arbitrage trades the module has executed |
| gRPC | merlins.protorev.Query/GetProtoRevProfitsByDenom | Queries the profits of the module by denom |
| gRPC | merlins.protorev.Query/GetProtoRevAllProfits | Queries all of the profits from the module |
| gRPC | merlins.protorev.Query/GetProtoRevStatisticsByRoute | Queries the number of arbitrages and profits that have been executed for a given route |
| gRPC | merlins.protorev.Query/GetProtoRevAllStatistics | Queries all of routes that the module has arbitrage against and the number of trades and profits that have been executed for each route |
| gRPC | merlins.protorev.Query/GetProtoRevTokenPairArbRoutes | Queries all of the hot routes that the module is currently arbitraging |
| gRPC | merlins.protorev.Query/GetProtoRevMaxPoolPointsPerTx | Queries the ProtoRev max pool points per transaction |
| gRPC | merlins.protorev.Query/GetProtoRevMaxPoolPointsPerBlock | Queries the ProtoRev max pool points per block |
| gRPC | merlins.protorev.Query/GetProtoRevAdminAccount | Queries the admin account of the ProtoRev |
| gRPC | merlins.protorev.Query/GetProtoRevDeveloperAccount | Queries the developer account of the ProtoRev |
| gRPC | merlins.protorev.Query/GetProtoRevBaseDenoms | Queries the ProtoRev base denoms used to create cyclic arbitrage routes |
| gRPC | merlins.protorev.Query/GetProtoRevEnabled | Queries whether the ProtoRev module is currently enabled |
| gRPC | merlins.protorev.Query/GetProtoRevPoolWeights | Queries the number of pool points each pool type will consume when executing and simulating trades |
| gRPC | merlins.protorev.Query/GetProtoRevPool | Queries the pool id for a given denom pair stored in ProtoRev |
| GET | /merlins/protorev/params | Queries the parameters of the module |
| GET | /merlins/protorev/number_of_trades | Queries the number of arbitrage trades the module has executed |
| GET | /merlins/protorev/profits_by_denom | Queries the profits of the module by denom |
| GET | /merlins/protorev/all_profits | Queries all of the profits from the module |
| GET | /merlins/protorev/statistics_by_route | Queries the number of arbitrages and profits that have happened for a given route |
| GET | /merlins/protorev/all_route_statistics | Queries all of routes that the module has arbitrage against and the number of trades and profits that have happened for each route |
| GET | /merlins/protorev/token_pair_arb_routes | Queries all of the hot routes that the module is currently arbitraging |
| GET | /merlins/protorev/max_pool_points_per_tx | Queries the maximum number of pool points that can be consumed per transaction |
| GET | /merlins/protorev/max_pool_points_per_block | Queries the maximum number of pool points that can be consumed per block |
| GET | /merlins/protorev/admin_account | Queries the admin account of the ProtoRev |
| GET | /merlins/protorev/developer_account | Queries the developer account of the ProtoRev |
| GET | /merlins/protorev/base_denoms | Queries the base denominations ProtoRev is currently using to create cyclic arbitrage routes |
| GET | /merlins/protorev/enabled | Queries whether the ProtoRev module is currently enabled |
| GET | /merlins/protorev/pool_weights | Queries the number of pool points each pool type will consume when executing and simulating trades |
| GET | /merlins/protorev/pool | Queries the pool id for a given denom pair stored in ProtoRev |

### Transactions

| Verb | Method | Description |
| --- | --- | --- |
| gRPC | merlins.protorev.Msg/SetHotRoutes | Sets the hot routes that will be explored when creating cyclic arbitrage routes. Can only be called by the admin account |
| gRPC | merlins.protorev.Msg/SetDeveloperAccount | Sets the account that can withdraw a portion of the profit from the ProtoRev module. Can only be called by the admin account |
| gRPC | merlins.protorev.Msg/SetMaxPoolPointsPerTx | Sets the maximum number of pool points that can be consumed per transaction |
| gRPC | merlins.protorev.Msg/SetMaxPoolPointsPerBlock | Sets the maximum number of routes that can be iterated per block |
| gRPC | merlins.protorev.Msg/SetBaseDenoms | Sets the base denominations the ProtoRev module will use to create cyclic arbitrage routes |
| gRPC | merlins.protorev.Msg/SetPoolWeights | Sets the amount of pool points each pool type will consume when executing and simulating trades |
| POST | /merlins/protorev/set_hot_routes | Sets the hot routes that will be explored when creating cyclic arbitrage routes. Can only be called by the admin account |
| POST | /merlins/protorev/set_developer_account | Sets the account that can withdraw a portion of the profit from the ProtoRev module. Can only be called by the admin account |
| POST | /merlins/protorev/set_max_pool_points_per_tx | Sets the maximum number of pool points that can be consumed per transaction |
| POST | /merlins/protorev/set_max_pool_points_per_block | Sets the maximum number of pool points that can be consumed per block |
| POST | /merlins/protorev/set_pool_weights | Sets the amount of pool points each pool type will consume when executing and simulating trades |
| POST | /merlins/protorev/set_base_denoms | Sets the base denominations that will be used by ProtoRev to construct cyclic arbitrage routes |
