...
func ValidateBaseDenoms(denoms []BaseDenom) error {
	// The first base denom must be the Merlins denomination
	if len(denoms) == 0 || denoms[0].Denom != MerlinsDenomination {
		return fmt.Errorf("the first base denom must be the Merlins denomination")
	}
