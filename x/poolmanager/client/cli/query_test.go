...
func (s *QueryTestSuite) TestQueriesNeverAlterState() {
	testCases := []struct {
		...
		{
			"Query num pools",
			"/merlins.poolmanager.v1beta1.Query/NumPools",
			&poolmanagerqueryproto.NumPoolsRequest{},
			&poolmanagerqueryproto.NumPoolsResponse{},
		},
		{
			"Query estimate swap in",
			"/merlins.poolmanager.v1beta1.Query/EstimateSwapExactAmountIn",
		...
		},
		{
			...
			"/merlins.poolmanager.v1beta1.Query/EstimateSwapExactAmountOut",
...
func (s *QueryTestSuite) TestSimplifiedQueries() {
	...
	err := s.QueryHelper.Invoke(gocontext.Background(),
		"/merlins.poolmanager.v1beta1.Query/EstimateSwapExactAmountIn", swapIn, output1)
	s.Require().NoError(err)
	err = s.QueryHelper.Invoke(gocontext.Background(),
		"/merlins.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountIn", simplifiedSwapIn, output2)
	s.Require().NoError(err)
	...
	err = s.QueryHelper.Invoke(gocontext.Background(),
		"/merlins.poolmanager.v1beta1.Query/EstimateSwapExactAmountOut", swapOut, output3)
	s.Require().NoError(err)
	err = s.QueryHelper.Invoke(gocontext.Background(),
		"/merlins.poolmanager.v1beta1.Query/EstimateSinglePoolSwapExactAmountOut", simplifiedSwapOut, output4)
	s.Require().NoError(err)
