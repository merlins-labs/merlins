...
import (
	...
	pooltypes "github.com/merlins-labs/merlins/v17/x/pool-incentives/types"
)
...
func (s *KeeperTestSuite) TestGRPCGaugeByID() {
	...
	res, err = s.querier.GaugeByID(sdk.WrapSDKContext(s.Ctx), &types.GaugeByIDRequest{Id: gaugeID})
...
func (s *KeeperTestSuite) TestGRPCGauges() {
	...
	res, err = s.querier.Gauges(sdk.WrapSDKContext(s.Ctx), &types.GaugesRequest{Pagination: &filter})
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestGRPCActiveGauges() {
	...
	res, err = s.querier.ActiveGauges(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesRequest{Pagination: &query.PageRequest{Limit: 5}})
	s.Require().NoError(err)
	s.Require().Len(res.Data, 5)

	// set page request limit to 15, expect only 10 active gauge responses
	res, err = s.querier.ActiveGauges(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesRequest{Pagination: &query.PageRequest{Limit: 15}})
	s.Require().NoError(err)
	s.Require().Len(res.Data, 10)
}
...
func (s *KeeperTestSuite) TestGRPCActiveGaugesPerDenom() {
	...
	res, err = s.querier.ActiveGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesPerDenomRequest{Denom: "lptoken", Pagination: nil})
	s.Require().NoError(err)
	s.Require().Len(res.Data, 1)
	expectedGauge := types.Gauge{
		Id:          gaugeID,
	...
	res, err = s.querier.ActiveGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesPerDenomRequest{Denom: "lptoken", Pagination: &query.PageRequest{Limit: 5}})
	s.Require().Len(res.Data, 1)
	s.Require().NoError(err)

	// query active gauges by pool denom with a page request of 5 should return 5 gauges
	res, err = s.querier.ActiveGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesPerDenomRequest{Denom: "pool", Pagination: &query.PageRequest{Limit: 5}})
	s.Require().Len(res.Data, 5)
	s.Require().NoError(err)

	// query active gauges by pool denom with a page request of 15 should return 10 gauges
	res, err = s.querier.ActiveGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.ActiveGaugesPerDenomRequest{Denom: "pool", Pagination: &query.PageRequest{Limit: 15}})
	s.Require().Len(res.Data, 10)
	s.Require().NoError(err)
}
...
func (s *KeeperTestSuite) TestGRPCUpcomingGauges() {
	...
	res, err = s.querier.UpcomingGauges(sdk.WrapSDKContext(s.Ctx), &types.UpcomingGaugesRequest{Pagination: &query.PageRequest{Limit: 5}})
	s.Require().NoError(err)
	s.Require().Len(res.Data, 5)
...
func (s *KeeperTestSuite) TestGRPCUpcomingGaugesPerDenom() {
	...
	res, err = s.querier.UpcomingGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.UpcomingGaugesPerDenomRequest{Denom: "lptoken", Pagination: &query.PageRequest{Limit: 5}})
	s.Require().NoError(err)
	s.Require().Len(res.UpcomingGauges, 0)

	// query upcoming gauges by pool denom with a page request of 5 should return 5 gauges
	res, err = s.querier.UpcomingGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.UpcomingGaugesPerDenomRequest{Denom: "pool", Pagination: &query.PageRequest{Limit: 5}})
	s.Require().NoError(err)
	s.Require().Len(res.UpcomingGauges, 5)

	// query upcoming gauges by pool denom with a page request of 15 should return 10 gauges
	res, err = s.querier.UpcomingGaugesPerDenom(sdk.WrapSDKContext(s.Ctx), &types.UpcomingGaugesPerDenomRequest{Denom: "pool", Pagination: &query.PageRequest{Limit: 15}})
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestGRPCRewardsEst() {
	...
	res, err := s.querier.RewardsEst(sdk.WrapSDKContext(s.Ctx), &types.RewardsEstRequest{
