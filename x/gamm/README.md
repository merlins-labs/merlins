...
The ``GAMM`` module (**G**eneralized **A**utomated **M**arket **M**aker) provides the logic to create and interact with liquidity pools on the Merlins DEX.
...
- Stableswap pools have liquidity centered around a given spot price. See [here](https://github.com/merlins-labs/merlins/blob/main/x/gamm/pool-models/stableswap/README.md) for the spec of the Merlins implementation.
...
[Spot price](https://github.com/merlins-labs/merlins/blob/main/x/gamm/keeper/swap.go)

#### Multi-Hop

The multi-hop logic is handled via `x/poolmanager` module.
Please see for details:
- https://github.com/merlins-labs/merlins/blob/main/x/poolmanager/README.md
...
3. **FutureGovernor** -
    Merlins plans to allow every pool to act as a DAO, with its own governance in a future upgrade. To facilitate this transition, we allow pools to specify who the governor should be as a string. There are currently 3 options for the future governor.
...
[MsgCreateBalancerPool](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/pool-models/balancer/tx.proto#L16-L26)

### MsgJoinPool

[MsgJoinPool](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L27-L39)

### MsgExitPool

[MsgExitPool](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L44-L57)

### MsgSwapExactAmountIn

[MsgSwapExactAmountIn](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L68-L80)
...
[MsgSwapExactAmountOut](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L90-L102)
...
[MsgJoinSwapExternAmountIn](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L107-L119)

#### MsgJoinSwapShareAmountOut

[MsgJoinSwapShareAmountOut](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L124-L138)

#### MsgExitSwapShareAmountIn

[MsgExitSwapShareAmountIn](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L143-L158)

#### MsgExitSwapExternAmountOut

[MsgExitSwapExternAmountOut](https://github.com/merlins-labs/merlins/blob/v7.1.0/proto/merlins/gamm/v1beta1/tx.proto#L163-L175)
...
```sh
merlins tx gamm create-pool [config-file] --from --chain-id
```
...
```sh
merlins tx gamm create-pool --pool-file [config-file] --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm join-pool --pool-id --max-amounts-in --share-amount-out --from --chain-id
```

::: details Example

Join `pool 3` with a **maximum** of `.037753 AKT` and the corresponding amount of `FURY` to get an **exact** share amount of `1.227549469722224220 gamm/pool/3` using `WALLET_NAME` on the merlins mainnet:

```sh
merlins tx gamm join-pool --pool-id 3 --max-amounts-in 37753ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --share-amount-out 1227549469722224220 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm exit-pool --pool-id --min-amounts-out --share-amount-in --from --chain-id
```

::: details Example

Exit `pool 3` with for **exactly** `1.136326462628731195 gamm/pool/3` in order to receive a **minimum** of `.033358 AKT` and the corresponding amount of `FURY` using `WALLET_NAME` on the merlins mainnet:

```sh
merlins tx gamm exit-pool --pool-id 3 --min-amounts-out 33358ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --share-amount-in 1136326462628731195 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm join-swap-extern-amount-in [token-in] [share-out-min-amount] --from --pool-id --chain-id
```

::: details Example

Join `pool 3` with **exactly** `.200000 AKT` (and `0 FURY`) to get a **minimum** of `3.234812471272883046 gamm/pool/3` using `WALLET_NAME` on the merlins mainnet:

```sh
merlins tx gamm join-swap-extern-amount-in 200000ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 3234812471272883046 --pool-id 3 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm exit-swap-extern-amount-out [token-out] [share-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlins tx gamm exit-swap-extern-amount-out 199430ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 3408979387886193586 --pool-id 3 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm join-swap-share-amount-out [token-in-denom] [share-out-amount] [token-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlins tx gamm join-swap-share-amount-out ufury 14481270389710236872 312466 --pool-id 3 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm exit-swap-share-amount-in [token-out-denom] [share-in-amount] [token-out-min-amount] --pool-id --from --chain-id
```
...
```sh
merlins tx gamm exit-swap-share-amount-in ufury 14563185400026723131 298548 --pool-id 3 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm swap-exact-amount-in [token-in] [token-out-min-amount] --pool-id --from --chain-id
```

::: details Example

Swap **exactly** `.407239 AKT` through `pool 3` into a **minimum** of `.140530 FURY` using `WALLET_NAME` on the merlins mainnet:

```sh
merlins tx gamm swap-exact-amount-in 407239ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 140530 --swap-route-pool-ids 3 --swap-route-denoms ufury --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm swap-exact-amount-out [token-out] [token-out-max-amount] --swap-route-pool-ids --from --chain-id
```

::: details Example

Swap a **maximum** of `.407239 AKT` through `pool 3` into **exactly** `.140530 FURY` using `WALLET_NAME` on the merlins mainnet:

```sh
merlins tx gamm swap-exact-amount-out 140530ufury 407239 --swap-route-pool-ids 3 --swap-route-denoms ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --from WALLET_NAME --chain-id merlins-1
```
...
```sh
merlins tx gamm migrate-position [unlocked-shares] [flags]
```
...
```sh
 merlins tx gamm migrate-position 10000000000000000000gamm/pool/2 --min-amounts-out=100ufury,100uusdc --from pool -b block --keyring-backend test --chain-id localmerlins --fees 1000000ufury --gas 700000
```
:::
## Queries
...
```sh
merlins query gamm estimate-swap-exact-amount-in <poolID> <sender> <tokenIn> [flags]
```
...
```sh
merlins query gamm estimate-swap-exact-amount-in 1 fury123nfq6m8f88m4g3sky570unsnk4zng4uqv7cm8 1000000ufury --swap-route-pool-ids 1 --swap-route-denoms ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 
```
...
```sh
merlins query gamm estimate-swap-exact-amount-out <poolID> <sender> <tokenOut> [flags]
```
...
```sh
merlins query gamm estimate-swap-exact-amount-out 1 fury123nfq6m8f88m4g3sky570unsnk4zng4uqv7cm8 1000000ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 --swap-route-pool-ids 1 --swap-route-denoms ufury
```
...
```sh
merlins query gamm pool <poolID> [flags]
```
...
```sh
merlins query gamm pool-assets <poolID> [flags]
```
...
```sh
merlins query gamm pool-assets 1
```
...
```sh
merlins query gamm pool-params <poolID> [flags]
```
...
```sh
merlins query gamm pool-params 1
```
...
```sh
merlins query gamm spot-price 1 ufury ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2
```
...
```sh
merlins query gamm total-liquidity
```
...
```sh
merlins query gamm total-share <poolID> [flags]
```
...
```sh
merlins query gamm total-share 1
```
