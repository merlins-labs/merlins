...
import (
	...
	"github.com/merlins-labs/merlins/v17/x/ibc-rate-limit/types"
)
...
func CalculateChannelValue(ctx sdk.Context, denom string, bankKeeper bankkeeper.Keeper) sdk.Int {
	...
	// For non-native (ibc) tokens, return the supply if the token in merlins
...
func (suite *MiddlewareTestSuite) initializeEscrow() (totalEscrow, expectedSed sdk.Int) {
	...
	supply := merlinsApp.BankKeeper.GetSupplyWithOffset(suite.chainA.GetContext(), sdk.DefaultBondDenom)
...
func (suite *MiddlewareTestSuite) fullSendTest(native bool) map[string]string {
	...
	channelValue := CalculateChannelValue(suite.chainA.GetContext(), denom, merlinsApp.BankKeeper)
...
func (suite *MiddlewareTestSuite) fullRecvTest(native bool) {
	...
	channelValue := CalculateChannelValue(suite.chainA.GetContext(), localDenom, merlinsApp.BankKeeper)
...
func (suite *MiddlewareTestSuite) TestFailedSendTransfer() {
	...
	// escrowed := merlinsApp.BankKeeper.GetBalance(suite.chainA.GetContext(), escrowAddress, sdk.DefaultBondDenom)
	escrowed := merlinsApp.BankKeeper.GetSupplyWithOffset(suite.chainA.GetContext(), sdk.DefaultBondDenom)
