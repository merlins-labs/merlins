		...
		s.Run(test.name, func() {
			...
			for _, pos := range test.setupFullyChargedPositions {
				...
				if pos.isLocked {
					...
					s.Require().NoError(err)
					liquidityCreated = fullRangePositionData.Liquidity
				} else {
...
func (s *KeeperTestSuite) TestNegativeTickRange_SpreadFactor() {
	...
	_, err := s.clk.CreateIncentive(s.Ctx, poolId, s.TestAccs[0], sdk.NewCoin("ufury", sdk.NewInt(1_000_000)), rewardsPerSecond, s.Ctx.BlockTime(), time.Nanosecond)
	s.Require().NoError(err)
	...
	// See issue: https://github.com/merlins-labs/merlins/issues/5854
