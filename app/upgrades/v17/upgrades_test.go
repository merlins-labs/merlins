...
func (suite *UpgradeTestSuite) TestUpgrade() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(keepers *keepers.AppKeepers) (sdk.Coins, uint64) {
				...
				for _, assetPair := range v17.AssetPairsForTestsOnly {
					...
					furyIn := sdk.NewCoin(v17.FURY, sdk.NewInt(100000).MulRaw(2))

					// Add the amount of fury that will be used to the expectedCoinsUsedInUpgradeHandler.
					expectedCoinsUsedInUpgradeHandler = expectedCoinsUsedInUpgradeHandler.Add(furyIn)
				...
				existingPool := suite.PrepareConcentratedPoolWithCoins("ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4", "ufury")
				existingPool2 := suite.PrepareConcentratedPoolWithCoins("akash", "ufury")
				existingBalancerPoolId := suite.PrepareBalancerPoolWithCoins(sdk.NewCoin("atom", sdk.NewInt(10000000000)), sdk.NewCoin("ufury", sdk.NewInt(10000000000)))

				// create few TWAP records for the pools
				t1 := dummyTwapRecord(existingPool.GetId(), time.Now().Add(-time.Hour*24), "ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4", "ufury", sdk.NewDec(10),
					sdk.OneDec().MulInt64(10*10),
					sdk.OneDec().MulInt64(3),
					sdk.ZeroDec())

				t2 := dummyTwapRecord(existingPool.GetId(), time.Now().Add(-time.Hour*10), "ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4", "ufury", sdk.NewDec(30),
					sdk.OneDec().MulInt64(10*10+10),
					sdk.OneDec().MulInt64(5),
					sdk.ZeroDec())

				t3 := dummyTwapRecord(existingPool.GetId(), time.Now().Add(-time.Hour), "ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4", "ufury", sdk.NewDec(20),
					sdk.OneDec().MulInt64(10*10+10*5),
					sdk.OneDec().MulInt64(10),
					sdk.ZeroDec())

				t4 := dummyTwapRecord(existingPool2.GetId(), time.Now().Add(-time.Hour*24), "akash", "ufury", sdk.NewDec(10),
					sdk.OneDec().MulInt64(10*10*10),
					sdk.OneDec().MulInt64(5),
					sdk.ZeroDec())

				t5 := dummyTwapRecord(existingPool2.GetId(), time.Now().Add(-time.Hour), "akash", "ufury", sdk.NewDec(20),
					sdk.OneDec().MulInt64(10),
					sdk.OneDec().MulInt64(2),
					sdk.ZeroDec())

				t6 := dummyTwapRecord(existingBalancerPoolId, time.Now().Add(-time.Hour), "atom", "ufury", sdk.NewDec(10),
					sdk.OneDec().MulInt64(10),
					sdk.OneDec().MulInt64(10),
					sdk.ZeroDec())

				t7 := dummyTwapRecord(existingBalancerPoolId, time.Now().Add(-time.Minute*20), "atom", "ufury", sdk.NewDec(50),
		...
		},
		{
			...
			func(keepers *keepers.AppKeepers) (sdk.Coins, uint64) {
				suite.Ctx = suite.Ctx.WithChainID("fury-test-5")
