...
func createCLPoolWithCommunityPoolPosition(ctx sdk.Context, keepers *keepers.AppKeepers, assetPair AssetPair, communityPoolAddress sdk.AccAddress) (clPoolDenom string, clPoolId uint64, poolLink gammmigration.BalancerToConcentratedPoolLink, coinsUsed sdk.Coins, err error) {
	...
	furyIn := sdk.NewCoin(FURY, sdk.NewInt(100000))
	nonFuryAsset := ""
	...
	_, err = keepers.GAMMKeeper.CalcOutAmtGivenIn(ctx, linkedClassicPool, furyIn, nonFuryAsset, assetPair.SpreadFactor)
	if err != nil {
		return "", 0, gammmigration.BalancerToConcentratedPoolLink{}, nil, err
	}
	...
	respectiveNonFuryAssetInt, err := keepers.GAMMKeeper.SwapExactAmountIn(ctx, communityPoolAddress, linkedClassicPool, furyIn, nonFuryAsset, sdk.ZeroInt(), linkedClassicPool.GetSpreadFactor(ctx))
	if err != nil {
		return "", 0, gammmigration.BalancerToConcentratedPoolLink{}, nil, err
	}
	ctx.Logger().Info(fmt.Sprintf("Swapped %s for %s%s from the community pool", furyIn.String(), respectiveNonFuryAssetInt.String(), nonFuryAsset))
	...
	fullRangeCoins := sdk.NewCoins(respectiveNonFuryAsset, furyIn)
