	...
	}{
		{
			...
			func() {
				...
				for i := uint64(1); i < v16.DaiOsmoPoolId; i++ {
					suite.PrepareBalancerPoolWithCoins(sdk.NewCoin("ufury", sdk.NewInt(10000000000)), sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", defaultDaiAmount))
				}

				// Create DAI / FURY pool
				suite.PrepareBalancerPoolWithCoins(sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", defaultDaiAmount), sdk.NewCoin("ufury", sdk.NewInt(10000000000)))
			...
			func() {
				...
				suite.Require().Equal(communityPoolBalancePre.AmountOf("ufury").Sub(respectiveOsmo.Amount).String(), communityPoolBalancePost.AmountOf("ufury").String())
